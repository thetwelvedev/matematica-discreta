...
def MATRIZ_COMBINA(n, lista):
    ...
    return lista_sem_zeros

def formatacao_saida(lista):
    print("{", end="")
    for i, sublista in enumerate(lista):
        print("{", end="")
        print(",".join(str(elemento) for elemento in sublista), end="")
        if i < len(lista) - 1:
            print("},", end="")
        else:
            print("}", end="")
...
formatacao_saida(MATRIZ_COMBINA(n, lista_trabalhada))
